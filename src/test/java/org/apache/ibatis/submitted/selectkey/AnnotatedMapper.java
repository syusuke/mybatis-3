...
public interface AnnotatedMapper {
  ...
  @Options(
      ...
      keyProperty = "nameId,generatedName",
  ...
  @Insert("insert into table2 (name) values(#{name})")
  @SelectKey(
      statement = "select id, name_fred from table2 where id = identity()",
      keyProperty = "nameId,generatedName",
      keyColumn = "ID,NAME_FRED",
      before = false,
  ...
  @Update("update table2 set name = #{name} where id = #{nameId}")
  @SelectKey(
      statement = "select name_fred from table2 where id = #{nameId}",
      keyProperty = "generatedName",
      keyColumn = "NAME_FRED",
      before = false,
  ...
  @Update("update table2 set name = #{name} where id = #{nameId}")
  @SelectKey(
      statement = "select name_fred as generatedName from table2 where id = #{nameId}",
      keyProperty = "generatedName",
      before = false,
      resultType = Name.class)
